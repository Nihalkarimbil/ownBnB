...
function Pendinglists() {
    ...
    const titleBodyTemplate = (product) => {
        return (
          <Link to={`/item-details/${product._id}`} className="text-blue-600 font-semibold">
    ...
    const footer = (
        <div className="p-4 text-sm text-gray-600 bg-gray-50 rounded-b-lg shadow-sm">
            Total products: {pending ? pending.length : 0}
        </div>
    );
    ...
    return (
        <div className="p-6 mt-16 ml-64 bg-white shadow-lg rounded-lg max-w-7xl mx-auto">
            <h1 className="text-2xl font-semibold text-gray-800 text-center" id='admdashbord'> pending Listings</h1>
            <DataTable
                value={pending}
                paginator
                first={first}
                rows={rows}
                onPage={onPageChange}
                totalRecords={pending ? pending.length : 0}
                footer={footer}
                stripedRows
