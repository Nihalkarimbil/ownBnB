...
function Reservation() {
  ...
  useEffect(() => {
    const fetchReservations = async () => {
      try {
        const res = await axiosinstance.get('/admin/admgetReserv');
  ...
  const footer = (
    <div className="p-4 text-sm text-gray-600 bg-gray-50 rounded-b-lg shadow-sm">
      Total Reservations: {reservations ? reservations.length : 0}
    </div>
  );

  return (
    <div className="p-4 mt-16 pl-72">
      <h2 className="text-2xl font-bold mb-4 text-center">Reservation List</h2>
      <DataTable
        value={reservations}
        footer={footer}
        paginator rows={10}
